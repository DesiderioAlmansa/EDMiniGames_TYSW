...
public class Manager {
	...
	private ConcurrentHashMap<String, WrapperSession> matchSessionsPorHttp;

	private ConcurrentHashMap<String, WrapperSession> matchSessionsPorWs;
	...
	private Manager() {
		...
		this.matchSessionsPorHttp = new ConcurrentHashMap<>();
		this.matchSessionsPorWs = new ConcurrentHashMap<>();
		try {
			loadParameters();
		} catch (Exception e) {
	...
	public void add(WrapperSession wrapperSession, String httpSessionId) {
		...
		this.matchSessionsPorHttp.put(httpSessionId, wrapperSession);
		this.matchSessionsPorWs.put(wrapperSession.getWsSession().getId(), wrapperSession);
	}
	...
	public UserRepository obtenerRepositorioUser() {
		return userRepo;
	}
	
	public MatchRepository obtenerRepositorioMatch() {
